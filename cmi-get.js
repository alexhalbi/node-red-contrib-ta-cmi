			...
			if (msg.httpStatusCode == 200) { // http connect sucessful
				if (msg.data["Status code"] == 0) { // cmi answer OK
					try { // check if all indexes are in the right range a.s.o.
						let newmsg = {};
						let statustext = '';
						// Parse CMI Output, config.item is stored in "Number" and does not represent index of array!
						var dataList = msg.data.Data[cmiSections[config.source]]
						var dataItem = dataList.filter(function(item) {
